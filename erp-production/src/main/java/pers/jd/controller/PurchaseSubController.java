...
public class PurchaseSubController {
    ...
    public ResultSet<Object> insertOrUpdate(@RequestBody PurchaseSub m) {

        ResultSet<Object> rs = new ResultSet<>();

        if (PurchaseSubService.saveOrUpdate(m)) {
            rs.setReturnCode(1);
            if (m.getId() == 0)
    ...
    public ResultSet<Object> delete(@RequestParam(value = "id") long id) {
        ResultSet<Object> rs = new ResultSet<>();
        if (PurchaseSubService.removeById(id)) {
            rs.setReturnCode(1);
    ...
    public PurchaseSub getOne(@RequestParam(value = "id") long id) {
        return PurchaseSubService.getById(id);
    }
    ...
    public List<PurchaseSub> getAll() {

        return PurchaseSubService.list();
    }
    ...
    public ResultGetAllPaging getAllPaging(@RequestParam(value = "pageIndex", defaultValue = "0")
        ...
        Page<PurchaseSub> page = new Page<>(pageIndex, pageSize);
        QueryWrapper<PurchaseSub> queryWrapper = new QueryWrapper<>();
        // queryWrapper.eq("price", 312312);

        List<PurchaseSub> dataList = PurchaseSubService.getAllPaging(page, queryWrapper);
        long countTotal = PurchaseSubService.count();
    ...
    public ResultGetAllPaging fuzzySearch(@RequestParam(value = "") String searchKey) {
        ...
        List<PurchaseSub> dataList = PurchaseSubService.list(queryWrapper);
    ...
    public ResultSet<Object> insertOrUpdateBatch(@RequestBody List<PurchaseSub> m_list) {

        ResultSet<Object> resultSet = new ResultSet<>();

        if (PurchaseSubService.saveOrUpdateBatch(m_list)) {
