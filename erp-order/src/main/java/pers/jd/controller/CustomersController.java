...
public class CustomersController {
    ...
    public ResultSet<Object> insertOrUpdate(@RequestBody Customers m) {

        ResultSet<Object> rs = new ResultSet<>();

        if (customersService.saveOrUpdate(m)) {
            rs.setReturnCode(1);
            if (m.getId() == 0)
    ...
    public ResultSet<Object> delete(@RequestParam(value = "id") long id) {
        ResultSet<Object> rs = new ResultSet<>();
        if (customersService.removeById(id)) {
            rs.setReturnCode(1);
    ...
    public Customers getOne(@RequestParam(value = "id") long id) {
    ...
    public ResultGetAllPaging getAllPaging(@RequestParam(value = "pageIndex", defaultValue = "0")
        ...
        List<Customers> dataList = customersService.getAllPaging(page, queryWrapper);
        long countTotal = customersService.count();
    ...
    public ResultGetAllPaging fuzzySearch(@RequestParam(value = "") String searchKey) {
        ...
        List<Customers> dataList = customersService.list(queryWrapper);
    ...
    public ResultSet<Object> insertOrUpdateBatch(@RequestBody List<Customers> m_list) {

        ResultSet<Object> resultSet = new ResultSet<>();

        if (customersService.saveOrUpdateBatch(m_list)) {
